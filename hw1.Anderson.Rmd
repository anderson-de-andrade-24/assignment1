...
Q1. For each of the following chunk options, explain what they do in one sentence (refer to the documentation linked above):
    a. echo: signals whether or not to show the source code in the output. 
    b. warning: signals whether or not to show the warnings in the output. 
    c. message:signals whether or not to show a message embedded within the code in your output. 
    d. tidy:can reformat the code. 
    e. error:how/if you want to see the errors in the output. 
...
ConvertedMatrix <- data.frame(matrix)
namesOfColumns <- c("x","y")
colnames(ConvertedMatrix) <- namesOfColumns

```

Q7. b. Next, use the ``$`` operator to select column ``y``. Print the twenty highest values from this column.
```{r q7b, echo=TRUE, tidy=TRUE}
yvalues <- sort(ConvertedMatrix$y, decreasing = TRUE)
print(head(yvalues, 20))
...
ConvertedMatrix$trials <- q$trials
...
# a
class(v) #it is character because fruit names are not numeric and cannot be read as numbers by R, therefore the numeric values included are converted to character values to match the fruits
...
## AI use
Please document any ways that you used ChatGPT or similar tools to help with this assignment:relied on codes in slides to create code
